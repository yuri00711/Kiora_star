...
function compactGroundingText(value: unknown): string {
  return normalizeGroundingText(value)
    .replace(/[\p{P}\p{S}\s]+/gu, "");
}

function evidenceIsGrounded(haystackValue: unknown, excerptValue: unknown): boolean {
  ...
  // Allow formatting-only differences such as Unicode punctuation, quotes,
  // dashes, line breaks, and full-width forms, but still reject paraphrases.
  const compactHaystack = compactGroundingText(haystack);
  const compactExcerpt = compactGroundingText(excerpt);
